 ...
 * 하나의 바구니에는 10개의 사과를 담을수 있다면 13개의 바구니가 필요할 것이다. (1)에 알맞은 코드를 넣으시오
 */
class Exec3_2 {
...
//아래는 화씨(F)를 섭씨(C)로 변환하는 코드이다 변환공식이 'C = (F − 32) / 1.8' 라고 할때 (1)에 알맞은 코드를 넣으시오. 단, 변환 결과값은 소수점 셋째자리에서 버림해야 한다. (형변환이용)
// 모든 문제는 코드 추가 가능 합니다.
class Exec3_7 {
	public static void main(String[] args) {
		int F = 100;
